...
const ACTORS = [
  ...
  { alias: 'd4rkn3t_brad', platforms: 2, last: '6h ago', risk: 'High', color: 'text-accent-blue', bg: 'bg-accent-blue' },
  { alias: 'xX_leakm4st', platforms: 3, last: '1d ago', risk: 'High', color: 'text-accent-blue', bg: 'bg-accent-blue' },
];

const TIMELINE = [
  { time: '14:47', label: 'Critical credential dump', color: 'bg-danger' },
  { time: '14:39', label: 'PII leak detected', color: 'bg-accent-blue' },
  { time: '14:35', label: 'API key exposure', color: 'bg-danger' },
  { time: '14:22', label: 'Threat resolved', color: 'bg-success' },
  { time: '13:58', label: 'New threat actor flagged', color: 'bg-accent-blue' },
];
...
function StatusChip({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    MONITOR: 'bg-accent-blue/10 text-accent-blue border-accent-blue/30',
...
function RiskBadge({ risk }: { risk: number }) {
  const color = risk >= 80 ? 'text-danger' : risk >= 50 ? 'text-accent-blue' : 'text-success';
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="rounded-[10px] overflow-hidden border border-border shadow-float relative">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border" style={{ background: 'linear-gradient(180deg, #0C1525 0%, #080F1C 100%)' }}>
            <div className="flex items-center gap-2">
              ...
              <div className="w-[7px] h-[7px] rounded-full bg-accent-blue/80" />
            ...
            <div className="flex-1 p-4 overflow-hidden">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                {[
                  ...
                  { border: 'border-t-accent-blue', icon: ShieldAlert, iconColor: 'text-accent-blue', value: '23', label: 'High Priority', trend: '▼ 5 from yesterday', trendColor: 'text-success' },
